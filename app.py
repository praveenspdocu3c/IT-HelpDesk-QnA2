...
if excel_files:
    ...
    if st.button("Get Resolution"):
        ...
        templatesp = """Provide a better search query for \
                    web search engine to answer the given question, end \
                    the queries with ’**’. Question: \
                    {x} Answer:"""

        rewrite_prompt = PromptTemplate(template=templatesp)

        # Pull the prompt from langsmith (replacement for hub)
        rewrite_prompt = client.pull_prompt("langchain-ai/rewrite")

        def _parse(text):
            return text.strip('"').strip("**")
                
        # Create and invoke the rewriter chain
        rewriter_chain = rewrite_prompt | llm | StrOutputParser() | _parse
        input_data = {"messages": [{"role": "user", "content": user_problem}]}
        rewritten_query = rewriter_chain.invoke(input_data)
